    ...
    <script type="text/javascript">
        ...
        $(function () {
            grid = $("#maingrid").ligerGrid({
                checkbox: true,
                columns: [
                    {display: '类别ID', name: 'id', align: 'left', width: 100},
                    {display: '类别名称', name: 'name', align: 'left', width: 100, minWidth: 60},
                    {display: '创建时间', name: 'createTime', width: 140}
                ],
                dataAction: 'server',
                url: 'manager/adCategory.do?method=adCategoryList',
                pageSize: 20,
                rownumbers: true,
                width: '100%',
                height: '100%',
        ...
        function addData(){
            $.ligerDialog.open({
                url: 'manager/ad/adCategoryAdd.jsp', height: 200, width: 300, buttons: [
                    {
                        ...
                        $.ajax({
                            cache: true,
                            type: "POST",
                            url:"manager/adCategory.do?method=saveAdCategory",
                            data: formData,
                            async: false,
                            error: function(data) {
                                $.ligerDialog.tip({
