...
function initSwfupload(url, sizeLimit, types, typsdesc, uploadLimit) {
	...
	var settings = {
		...
		flash_url: contextPath + "/resources/swf/swfupload.swf",
		...
		// Button Settings
		button_image_url: contextPath + "/resources/images/select-files-zh_CN.png",
...
function init(uploadLimit,sizeLimit){
	if($(".swf-ke-dialog-upload").length>0){
		$(".swf-ke-dialog-upload").remove();
	}
	var uploadbody='<div id="uploadBody" class="swf-ke-dialog-upload">'
		+'<div id="uploadHead" class="swf-ke-dialog-upload-head">'
		+'  <div style="float: left">批量图片上传</div><div id="iconClose" class="swf-ke-dialog-icon-close" title="关闭"></div><div style="clear: both"></div>'
		+'	</div>'
		+'	<!--content-->'
		+'	<div class="swf-ke-dialog-content">'
		+'		<div class="swf-ke-dialog-content-head">'
		+'		  <div id="errorMsg" class="swf-ke-dialog-content-error"></div>'
		+'		  <div id="swfbuttonParent" class="swf-ke-swfupload-button"><span id="buttonPlaceholder" /></div>'
		+'		  <div class="swf-ke-inline-block swf-ke-swfupload-desc">允许用户同时上传&nbsp;<font color="red">'+uploadLimit+'</font>&nbsp;张图片，单张图片容量不超过<font color="red">'+sizeLimit+'</font></div>'
		+'			<div class="swf-ke-button-common swf-ke-button-outer swf-ke-swfupload-startupload"><input id="btnUpload" type="button" class="swf-ke-button-common swf-ke-button" value="开始上传"></div>'
		+'			<div style="clear: both;"></div>'		
		+' 		</div>'
		+'		<div id="photoContainer" class="swf-ke-swfupload-body">'
		+'		</div>'
		+'	</div>'
		+'	<!--bottom-->'
		+'	<div class="swf-ke-dialog-footer">'
		+'	<span class="swf-ke-button-common swf-ke-button-outer" title="全部插入"><input id="insertAll" class="swf-ke-button-common swf-ke-button" type="button" value="全部插入"></span>'
		+'	<span class="swf-ke-button-common swf-ke-button-outer" title="全部清空"><input id="clearAll" class="swf-ke-button-common swf-ke-button" type="button" value="全部清空"></span>'
		+'	<span class="swf-ke-button-common swf-ke-button-outer" title="取消"><input id="btncancel" class="swf-ke-button-common swf-ke-button" type="button" value="取消"></span></div>'
		+'	</div>'
		+' <div style="display: block; width: 100%; height: 1200px; position: absolute; left: 0px; top: 0px; z-index: 811212;" class="swf-ke-dialog-mask"></div>';
...
function photoMouseOver() {
	$(this).addClass("swf-ke-on");
}
function photoMouseOut(obj) {
	$(this).removeClass("swf-ke-on");
}
...
function uploadStart(file) {
	$("#" + file.id).find(".swf-ke-progressbar").css("display", "block");
	$("#" + file.id).find(".swf-ke-message").css("display", "none");
}
// 上传的进度
...
function uploadSuccess(file, serverData) {
	try {
		...
		$("#" + file.id).find(".swf-ke-img").attr("data-status", data.status);
		if (data.status == 0 || data.status == "0") {
			$("#" + file.id).find(".swf-ke-img").attr("src", contextPath + data.message);
...
function addReadyFileInfo(fileid, fileName, message) {
	var photoDiv = '<div class="swf-ke-inline-block swf-ke-item" id="' + fileid + '">'
		+ '<div class="swf-ke-inline-block swf-ke-photo" style="height:80px; width:80px;">'
		+ '	<img src="' + contextPath + '/resources/images/image.png" class="swf-ke-img" data-status="-1" width="80" height="80" />'
		+ '		<span class="swf-ke-delete"></span>'
		+ '			<div class="swf-ke-status">'
		+ '				<div class="swf-ke-progressbar" style="display: none;">'
		+ '          		 <div class="swf-ke-progressbar-bar">'
		+ '               	 	<div class="swf-ke-progressbar-bar-inner"></div>'
		+ '           		 </div>'
		+ '            		<div class="swf-ke-progressbar-percent">0%</div>'
		+ '        		 </div>'
		+ '				<div class="swf-ke-message">' + message + '</div>'
		+ '   	 	</div>'
		+ '    	</div>'
		+ '  <div class="swf-ke-name" alt="' + fileName + '">' + fileName + '</div>'
		+ '</div>';

	$("#" + swfu.customSettings.photoContainer_Id).append(photoDiv);
	$("#" + fileid).find(".swf-ke-photo").bind("mouseover", photoMouseOver).bind("mouseout", photoMouseOut);
	$("#" + fileid).find(".swf-ke-delete").bind("click", function () {
		deleteFile(fileid)
	});
}
...
function FileProgress(file) {
	var fileID = file.id;
	this.fileProgressWrapper = $("#"+file.id).find(".swf-ke-status"); //document.getElementById(this.fileProgressID);
...
FileProgress.prototype.setProgress = function (percentage) {
	$(this.fileProgressWrapper).find(".swf-ke-progressbar-bar-inner").css("width",percentage + "%");
	$(this.fileProgressWrapper).find(".swf-ke-progressbar-percent").text(percentage + "%");
};

FileProgress.prototype.setComplete = function () {
	$(this.fileProgressWrapper).find(".swf-ke-progressbar-bar-inner").css("width","");
};
...
FileProgress.prototype.setStatus = function (status) {
	$(this.fileProgressWrapper).find(".swf-ke-message").text(status);
};

FileProgress.prototype.toggleCancel = function (show) {
	if(typeof show =='undefined'){
		$(this.fileProgressWrapper).find(".swf-ke-progressbar").css("display", "none");
		$(this.fileProgressWrapper).find(".swf-ke-message").css("display","none");
	}else{
		$(this.fileProgressWrapper).find(".swf-ke-progressbar").css("display", show ? "block" : "none");
		$(this.fileProgressWrapper).find(".swf-ke-message").css("display", show ? "none" : "block");
	}
};
